import json, base64, requests, os, traceback
...
class ClipAnalyze(object):
    ...
    def __init__(self):
        ...
        self.local_pic_url = info["azure"]["clip_pic_url"]
        self.analyze_json = {}
    ...
    def tencent_pic(self, image_url_list):
        ...
        try:
            ...
            for count, image_url in enumerate(image_url_list):
                ...
                with open(image_url + ".jpg", 'rb') as f:
    ...
    def azure_pic(self, image_url_list):
        ...
        for count, image_url in enumerate(image_url_list):
            data = {
                "url": self.local_pic_url + image_url.split("/")[-1] + ".jpg"
            }
            try:
                ...
                if response:
                    ...
                    if image_tag["description"]["tags"]:
                        ...
                        for i in range(tag_length):
                            azure_tag_set.add(image_tag["description"]["tags"][i].encode('utf8').decode())
                    if image_tag["description"]["captions"]:
                        for sentence in image_tag["description"]["captions"]:
                            if sentence["confidence"] >= 0.1:
                                azure_tag_set.add(sentence["text"].encode('utf8').decode())
